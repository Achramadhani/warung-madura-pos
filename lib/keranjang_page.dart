                    ...
                    onPressed: () async {
                      ...
                      if (cart.items.isNotEmpty) {
                        try {
                          ...
                          final transactionId = await DbHelper.instance
                              .simpanTransaksi(cart.totalHarga, cart.items);

                          // 2. Ambil data transaksi yang baru disimpan
                          final transactionHeader = await DbHelper.instance
                              .getAllTransaksi()
                              .then((headers) => headers
                                  .firstWhere((h) => h['id'] == transactionId));
                          final transactionDetails = await DbHelper.instance
                              .getDetailTransaksiByTransaksiId(transactionId);

                          final transactionData = {
                            'id': transactionId,
                            'code': '#SS-${(transactionId + 99278).toString()}',
                            'tgl_transaksi': transactionHeader['tgl_transaksi'],
                            'total_harga': transactionHeader['total_harga'],
                            'metode_bayar':
                                transactionHeader['metode_bayar'] ?? 'Tunai',
                            'details': transactionDetails,
                          };

                          // 3. Kosongkan Keranjang di Provider
                          cart.clearCart();

                          if (!context.mounted) return;

                          // 4. Navigasi ke halaman detail transaksi
